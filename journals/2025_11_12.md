	...
	- Find a proposed index k on the x-axis
	  logseq.order-list-type:: number
		...
		- Otherwise split and your new area is:
		  logseq.order-list-type:: number
			- The standard interpolation-search probe position x within indices low and high for key k in an array A (with A[low] ≤ k ≤ A[high]) is:
			  logseq.order-list-type:: number
...
- When is presorting worth it? #card
	- Closest pair – Θ(n2) → Θ (n log n), so presort
	- Checking uniqueness – Brute force is Θ (n2 ), so presort
	- Finding the min and max – Brute force is Θ (n), so NO presort
	- Finding the median – Brute force is Θ (n2 ), so presort
	- Searching unordered array – Brute force is Θ (n), so NO presort
