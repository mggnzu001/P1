...
I will regenerate your full notes so that **every flash card is a bullet starting with `-`**, nested properly, while the main narrative remains as normal Markdown. All previous `*` bullets inside cards will be converted to `-` for Logseq compatibility.

Here’s the cleaned-up, **Logseq-compatible version**:
...
* No general program can predict arbitrary program halting
...
* SAT, 3SAT, Clique, Vertex Cover, Hamiltonian Path, TSP (decision), Subset Sum
